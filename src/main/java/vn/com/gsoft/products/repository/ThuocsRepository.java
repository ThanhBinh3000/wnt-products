...
public interface ThuocsRepository extends BaseRepository<Thuocs, ThuocsReq, Long> {
    ...
    @Query("SELECT distinct c FROM Thuocs c " +
            ...
            + " OR (:#{#param.textSearch} IS NULL OR lower(c.tenThuoc) LIKE lower(concat('%',CONCAT(:#{#param.textSearch},'%')))) "
            + " OR (:#{#param.textSearch} IS NULL OR lower(c.thongTin) LIKE lower(concat('%',CONCAT(:#{#param.textSearch},'%'))))) "
            + " AND (:#{#param.nhaThuocMaNhaThuoc} IS NULL OR c.id in (select e.id from PhieuKiemKes d JOIN PhieuKiemKeChiTiets e on e.phieuKiemKeMaPhieuKiemKe = d.id where d.nhaThuocMaNhaThuoc= :#{#param.nhaThuocMaNhaThuoc} AND e.thuocThuocId = d.id AND d.daCanKho = false AND e.recordStatusId =0 AND d.recordStatusId =0)) "
            + " ORDER BY c.id desc"
    )
