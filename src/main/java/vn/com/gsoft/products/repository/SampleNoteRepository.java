...
public interface SampleNoteRepository extends BaseRepository<SampleNote, SampleNoteReq, Long> {
    @Query("SELECT c FROM SampleNote c " +
            ...
            + " AND (:#{#param.fromDateNote} IS NULL OR c.noteDate >= :#{#param.fromDateNote})"
            + " AND (:#{#param.toDateNote} IS NULL OR c.noteDate <= :#{#param.toDateNote})"
            + " AND (:#{#param.isConnect} IS NULL OR c.isConnect = :#{#param.isConnect})"
            + " ORDER BY c.id desc"
    )
