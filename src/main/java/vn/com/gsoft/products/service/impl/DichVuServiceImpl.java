...
import vn.com.gsoft.products.service.DichVuService;
...
@Service
@Log4j2
public class DichVuServiceImpl extends BaseServiceImpl<Thuocs, ThuocsReq, Long> implements DichVuService {

    private ThuocsRepository hdrRepo;

    @Autowired
    public DichVuServiceImpl(ThuocsRepository hdrRepo) {
        super(hdrRepo);
    ...
    public Thuocs create(ThuocsReq req) throws Exception {
        ...
        if (!maThuoc.isEmpty()) {
            ...
            if (byMaThuocAndRecordStatusId.isPresent()) {
                throw new Exception("Đã tồn tại mã dịch vụ ");
            }
        }
        //check trùng tên
        if (!tenThuoc.isEmpty()) {
            ...
            if (byTenThuocAndRecordStatusId.isPresent()) {
                throw new Exception("Đã tồn tại tên dịch vụ .");
            }
        }
        //check trùng barcode
        if (!barCode.isEmpty()) {
            ...
            if (byBarcodeAndRecordStatusId.isPresent()) {
                throw new Exception("Đã tồn tại mã vạch dịch vụ .");
            }
        }
